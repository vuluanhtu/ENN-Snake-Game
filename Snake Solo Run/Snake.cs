...
namespace Snake_Solo_Run
{
    public class Snake
    {
		...
		public float score = 0; //= Snake.score;
        public int die = 0; //= Snake.die;

        private float wallUp; //= 1-this.py/(this.tc-1);
        private float wallDown; //= this.py/(this.tc-1);
        private float wallLeft; //= 1-this.px/(this.tc-1);
        private float wallRight; // = this.px/(this.tc-1);
        ...
        public void StartGame(int a)
        {
            ...
            //document.addEventListener("keydown", Snake.keyPush); // when keydown, call function keyPush
            //Decision(a);
            //Snake.intervalId = setInterval(Snake.Game,100,canv, ctx, a);
        ...
        public float[] GetInput()
        {
            ...
            //Normalize data: squeeze data between 0 (far) and 1 (close). I dont know why my logic is reversed in direction
            foodUp = 1 - minDown / (tc - 1); //I dont know why my logic makes sense but the results are reversed in directions so the quick fixes are like these.
